...
v1.7 在数据集版本管理基础上增强质量评估。系统保留原有 `scores` 兼容字段，同时为每条 conversation 生成 `quality_report`，包含质量等级、通过/拒绝决策、多 Judge 投票、维度诊断、薄弱点和改进动作。
...
- 数据集版本创建、列表、详情、删除和按版本导出
- 增强质量评估：Quality Report、多 Judge 投票、维度诊断和改进建议
- 数据去重与多样性控制：为每条 conversation 生成 content_hash、duplicate_of、similarity_score 和 diversity_report
- 数据集版本记录 duplicate_count、duplicate_rate 和 diversity_score
...
DELETE /api/datasets/versions/{version_id}
```
...
```text
筛选 conversation
-> POST /api/datasets/versions 固化为版本
-> 保存 filters + conversation_ids 快照
-> GET /api/datasets/versions 查看版本列表
-> GET /api/datasets/versions/{version_id}/export.jsonl 导出指定版本
```
...
把当前筛选条件命中的 conversation 固化为一个数据集版本。

```json
{
  "name": "code-review-security-v1",
  "description": "安全审查方向高质量样本",
  "scenario": "code_review",
  "accepted": true,
  "min_score": 7,
  "q": "security"
...
### GET /api/datasets/versions/{version_id}/export.jsonl

导出指定版本的 JSONL 数据。

### DELETE /api/datasets/versions/{version_id}

删除数据集版本记录，不删除原始 conversation。
